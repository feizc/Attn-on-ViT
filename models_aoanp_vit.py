...
class NonParamAoA(nn.Module):
    ...
    def forward(self, x):
        ...
        # Q (bsz, num_head, (seq_len, d_model// num_head)) 
        # * W in last,  average in seq_len 
        
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn) # (bsz, num_heads, seq_len, seq_len)  -> (bsz, num_heads, seq_len, d_model// num_head)

        x = (attn @ v).transpose(1, 2) # (bsz, seq_len, n_h, d_model/ n_h)
        attn_head = (x @ x.transpose(-2, -1))
        attn_head = attn_head.softmax(dim=-1) 
        x = attn_head @ x # (bsz, seq_len, n_h, d_model/ n_h)
        x = x.reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = NonParamAoA(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
...
def aoa_vit_base_patch16(**kwargs):
...
def aoa_vit_large_patch16(**kwargs):
    model = AttenOnAttenVisionTransformer(
...
def aoa_vit_huge_patch14(**kwargs):
    model = AttenOnAttenVisionTransformer(
